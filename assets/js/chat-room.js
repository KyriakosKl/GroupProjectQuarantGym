...
ChatRoom.prototype.init = function(messages) {
  ...
  function sendChat (cb) {
    ...
    io.socket.post('/chatmessage', { userId: self.loggedInUserId, text: msg}, function(body, response) {
...
ChatRoom.prototype.renderMessage = function(message, scroll) {
  ...
  if (message.userId) {
    let user1 = message.userId.firstName ? message.userId.firstName : ""
    chatLineEl = $('<div class="chat-line"><span class="username">' + user1 + ':&nbsp;</span><span class="text">' + message.text + '</span></div>');
  }
